using System;
using System.Threading.Tasks;
using Newtonsoft.Json;
using TdLib.TdApi.Objects;
// REUSE-IgnoreStart

namespace TdLib.TdApi;
/// <summary>
/// Recovers the 2-step verification password with a password recovery code sent to an email address that was previously set up. Works only when the current authorization state is authorizationStateWaitPassword
/// </summary>
public class RecoverAuthenticationPassword : Function<Ok>
{
    /// <summary>
    /// Data type for serialization
    /// </summary>
    [JsonProperty("@type")]
    public override string DataType { get; set; } = "recoverAuthenticationPassword";

    /// <summary>
    /// Extra data attached to the function
    /// </summary>
    [JsonProperty("@extra")]
    public override string Extra { get; set; }

            /// <summary>
            /// Recovery code to check
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("recovery_code")]
            public string RecoveryCode { get; set; }

            /// <summary>
            /// New 2-step verification password of the user; may be empty to remove the password
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("new_password")]
            public string NewPassword { get; set; }

            /// <summary>
            /// New password hint; may be empty
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("new_hint")]
            public string NewHint { get; set; }
}
/// <summary>
/// Autogenerated TDLib APIs
/// </summary>
public static partial class ClientExtensions
{
    /// <summary>
    /// Recovers the 2-step verification password with a password recovery code sent to an email address that was previously set up. Works only when the current authorization state is authorizationStateWaitPassword
    /// </summary>
    public static Task<Ok> RecoverAuthenticationPasswordAsync(
        this Client client, string recoveryCode = default, string newPassword = default, string newHint = default)
    {
        return client.ExecuteAsync(new RecoverAuthenticationPassword
        {
            RecoveryCode = recoveryCode, NewPassword = newPassword, NewHint = newHint
        });
    }
}
public partial class Client
{
    /// <summary>
    /// Recovers the 2-step verification password with a password recovery code sent to an email address that was previously set up. Works only when the current authorization state is authorizationStateWaitPassword
    /// </summary>
    public Task<Ok> RecoverAuthenticationPasswordAsync(string recoveryCode = default, string newPassword = default, string newHint = default)
    {
    //test
        return ExecuteAsync(new RecoverAuthenticationPassword
        {
            RecoveryCode = recoveryCode, NewPassword = newPassword, NewHint = newHint
        });
    }
}
public partial interface IClient
{
    /// <summary>
    /// Recovers the 2-step verification password with a password recovery code sent to an email address that was previously set up. Works only when the current authorization state is authorizationStateWaitPassword
    /// </summary>
    Task<Ok> RecoverAuthenticationPasswordAsync(string recoveryCode = default, string newPassword = default, string newHint = default);
}


// REUSE-IgnoreEnd