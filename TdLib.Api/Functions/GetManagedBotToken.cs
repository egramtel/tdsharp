using System;
using System.Threading.Tasks;
using Newtonsoft.Json;

// REUSE-IgnoreStart
namespace TdLib
{
    /// <summary>
    /// Autogenerated TDLib APIs
    /// </summary>
    public static partial class TdApi
    {
        /// <summary>
        /// Returns token of a managed bot; for bots only
        /// </summary>
        public class GetManagedBotToken : Function<Text>
        {
            /// <summary>
            /// Data type for serialization
            /// </summary>
            [JsonProperty("@type")]
            public override string DataType { get; set; } = "getManagedBotToken";

            /// <summary>
            /// Extra data attached to the function
            /// </summary>
            [JsonProperty("@extra")]
            public override string Extra { get; set; }

            /// <summary>
            /// Identifier of the managed bot
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("bot_user_id")]
            public long BotUserId { get; set; }

            /// <summary>
            /// Pass true to revoke the current token and create a new one
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("revoke")]
            public bool Revoke { get; set; }
        }

        /// <summary>
        /// Returns token of a managed bot; for bots only
        /// </summary>
        public static Task<Text> GetManagedBotTokenAsync(
            this IClient client, long botUserId = default, bool revoke = default)
        {
            return client.ExecuteAsync(new GetManagedBotToken
            {
                BotUserId = botUserId, Revoke = revoke
            });
        }
    }
}
// REUSE-IgnoreEnd