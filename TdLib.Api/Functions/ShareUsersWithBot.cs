using System;
using System.Threading.Tasks;
using Newtonsoft.Json;
using TdLib.TdApi.Objects;
// REUSE-IgnoreStart

namespace TdLib.TdApi;
/// <summary>
/// Shares users after pressing a keyboardButtonTypeRequestUsers button with the bot
/// </summary>
public class ShareUsersWithBot : Function<Ok>
{
    /// <summary>
    /// Data type for serialization
    /// </summary>
    [JsonProperty("@type")]
    public override string DataType { get; set; } = "shareUsersWithBot";

    /// <summary>
    /// Extra data attached to the function
    /// </summary>
    [JsonProperty("@extra")]
    public override string Extra { get; set; }

            /// <summary>
            /// Identifier of the chat with the bot
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("chat_id")]
            public long ChatId { get; set; }

            /// <summary>
            /// Identifier of the message with the button
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("message_id")]
            public long MessageId { get; set; }

            /// <summary>
            /// Identifier of the button
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("button_id")]
            public int ButtonId { get; set; }

            /// <summary>
            /// Identifiers of the shared users
            /// </summary>
            [JsonProperty("shared_user_ids", ItemConverterType = typeof(Converter))]
            public long[] SharedUserIds { get; set; }

            /// <summary>
            /// Pass true to check that the users can be shared by the button instead of actually sharing them
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("only_check")]
            public bool OnlyCheck { get; set; }
}
/// <summary>
/// Autogenerated TDLib APIs
/// </summary>
public static partial class ClientExtensions
{
    /// <summary>
    /// Shares users after pressing a keyboardButtonTypeRequestUsers button with the bot
    /// </summary>
    public static Task<Ok> ShareUsersWithBotAsync(
        this Client client, long chatId = default, long messageId = default, int buttonId = default, long[] sharedUserIds = default, bool onlyCheck = default)
    {
        return client.ExecuteAsync(new ShareUsersWithBot
        {
            ChatId = chatId, MessageId = messageId, ButtonId = buttonId, SharedUserIds = sharedUserIds, OnlyCheck = onlyCheck
        });
    }
}
public partial class Client
{
    /// <summary>
    /// Shares users after pressing a keyboardButtonTypeRequestUsers button with the bot
    /// </summary>
    public Task<Ok> ShareUsersWithBotAsync(long chatId = default, long messageId = default, int buttonId = default, long[] sharedUserIds = default, bool onlyCheck = default)
    {
    //test
        return ExecuteAsync(new ShareUsersWithBot
        {
            ChatId = chatId, MessageId = messageId, ButtonId = buttonId, SharedUserIds = sharedUserIds, OnlyCheck = onlyCheck
        });
    }
}
public partial interface IClient
{
    /// <summary>
    /// Shares users after pressing a keyboardButtonTypeRequestUsers button with the bot
    /// </summary>
    Task<Ok> ShareUsersWithBotAsync(long chatId = default, long messageId = default, int buttonId = default, long[] sharedUserIds = default, bool onlyCheck = default);
}


// REUSE-IgnoreEnd