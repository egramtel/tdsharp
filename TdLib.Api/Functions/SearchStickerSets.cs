using System;
using System.Threading.Tasks;
using Newtonsoft.Json;

// REUSE-IgnoreStart
namespace TdLib
{
    /// <summary>
    /// Autogenerated TDLib APIs
    /// </summary>
    public static partial class TdApi
    {
        /// <summary>
        /// Searches for sticker sets by looking for specified query in their title and name. Excludes installed sticker sets from the results
        /// </summary>
        public class SearchStickerSets : Function<StickerSets>
        {
            /// <summary>
            /// Data type for serialization
            /// </summary>
            [JsonProperty("@type")]
            public override string DataType { get; set; } = "searchStickerSets";

            /// <summary>
            /// Extra data attached to the function
            /// </summary>
            [JsonProperty("@extra")]
            public override string Extra { get; set; }

            /// <summary>
            /// Type of the sticker sets to return
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("sticker_type")]
            public StickerType StickerType { get; set; }

            /// <summary>
            /// Query to search for
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("query")]
            public string Query { get; set; }
        }

        /// <summary>
        /// Searches for sticker sets by looking for specified query in their title and name. Excludes installed sticker sets from the results
        /// </summary>
        public static Task<StickerSets> SearchStickerSetsAsync(
            this Client client, StickerType stickerType = default, string query = default)
        {
            return client.ExecuteAsync(new SearchStickerSets
            {
                StickerType = stickerType, Query = query
            });
        }
          public partial class Client
            {
                /// <summary>
                /// Searches for sticker sets by looking for specified query in their title and name. Excludes installed sticker sets from the results
                /// </summary>
                public Task<StickerSets> SearchStickerSetsAsync(StickerType stickerType = default, string query = default)
                {
                //test
                    return ExecuteAsync(new SearchStickerSets
                    {
                        StickerType = stickerType, Query = query
                    });
                }
            }
            public partial interface IClient
            {
                /// <summary>
                /// Searches for sticker sets by looking for specified query in their title and name. Excludes installed sticker sets from the results
                /// </summary>
                Task<StickerSets> SearchStickerSetsAsync(StickerType stickerType = default, string query = default);
            }
    }


}
// REUSE-IgnoreEnd