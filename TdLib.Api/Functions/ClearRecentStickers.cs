using System;
using System.Threading.Tasks;
using Newtonsoft.Json;
using TdLib.TdApi.Objects;
// REUSE-IgnoreStart

namespace TdLib.TdApi;
/// <summary>
/// Clears the list of recently used stickers
/// </summary>
public class ClearRecentStickers : Function<Ok>
{
    /// <summary>
    /// Data type for serialization
    /// </summary>
    [JsonProperty("@type")]
    public override string DataType { get; set; } = "clearRecentStickers";

    /// <summary>
    /// Extra data attached to the function
    /// </summary>
    [JsonProperty("@extra")]
    public override string Extra { get; set; }

            /// <summary>
            /// Pass true to clear the list of stickers recently attached to photo or video files; pass false to clear the list of recently sent stickers
            /// </summary>
            [JsonConverter(typeof(Converter))]
            [JsonProperty("is_attached")]
            public bool IsAttached { get; set; }
}
/// <summary>
/// Autogenerated TDLib APIs
/// </summary>
public static partial class ClientExtensions
{
    /// <summary>
    /// Clears the list of recently used stickers
    /// </summary>
    public static Task<Ok> ClearRecentStickersAsync(
        this Client client, bool isAttached = default)
    {
        return client.ExecuteAsync(new ClearRecentStickers
        {
            IsAttached = isAttached
        });
    }
}
public partial class Client
{
    /// <summary>
    /// Clears the list of recently used stickers
    /// </summary>
    public Task<Ok> ClearRecentStickersAsync(bool isAttached = default)
    {
    //test
        return ExecuteAsync(new ClearRecentStickers
        {
            IsAttached = isAttached
        });
    }
}
public partial interface IClient
{
    /// <summary>
    /// Clears the list of recently used stickers
    /// </summary>
    Task<Ok> ClearRecentStickersAsync(bool isAttached = default);
}


// REUSE-IgnoreEnd