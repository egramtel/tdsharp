...
let processUpdates (update : TdApi.Update) = task {
    ...
    | :? TdApi.Update.UpdateAuthorizationState as update ->
        ...
        | :? TdApi.AuthorizationState.AuthorizationStateWaitTdlibParameters ->
            let param = TdApi.TdlibParameters(ApiId = apiId, ApiHash = apiHash, ApplicationVersion = applicationVersion,
                DeviceModel = "PC", SystemLanguageCode = "en",
                DatabaseDirectory = filesLocation, FilesDirectory = filesLocation)
            do! client.ExecuteAsync(TdApi.SetTdlibParameters(Parameters = param)) :> Task
        | :? TdApi.AuthorizationState.AuthorizationStateWaitEncryptionKey ->
            do! client.ExecuteAsync(TdApi.CheckDatabaseEncryptionKey()) :> Task
...
let main() = task {
    ...
    printfn $"Successfully logged in as [{currentUser.Id}] / [@{currentUser.Username}] / [{fullUserName}]"
